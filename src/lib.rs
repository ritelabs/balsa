...
```rust
# use slog::{Drain, o};
# use balsa::{Config, storage::MemStorage, raw_node::RawNode};
...
```rust
# use balsa::{Config, storage::MemStorage, raw_node::RawNode, eraftpb::Message};
# use std::{
...
other nodes:
    ...
    ```rust
    # use slog::{Drain, o};
    # use balsa::{Config, storage::MemStorage, raw_node::RawNode, StateRole};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
need to update the applied index and resume `apply` later:
    ...
    ```rust
    # use slog::{Drain, o};
    # use balsa::{Config, storage::MemStorage, raw_node::RawNode, eraftpb::EntryType};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
    ...
    # let mut ready = node.ready();
    #
    # fn handle_conf_change(e:  balsa::eraftpb::Entry) {
    # }
    #
    # fn handle_conf_change_v2(e:  balsa::eraftpb::Entry) {
    # }
    #
    # fn handle_normal(e:  balsa::eraftpb::Entry) {
    # }
    #
...
to advance the applied index inside.
    ...
    # use slog::{Drain, o};
    # use balsa::{Config, storage::MemStorage, raw_node::RawNode};
    # use balsa::eraftpb::{EntryType, Entry, Message};
    #
    # let config = Config { id: 1, ..Default::default() };
    # config.validate().unwrap();
    # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
```no_run
# use balsa::{Config, storage::MemStorage, raw_node::RawNode, eraftpb::*};
...
let steps = vec![
    balsa_proto::new_conf_change_single(4, ConfChangeType::AddNode),
    balsa_proto::new_conf_change_single(3, ConfChangeType::RemoveNode),
];
